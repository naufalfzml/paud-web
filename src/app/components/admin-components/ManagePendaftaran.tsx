...
const Notification: FC<{
  ...
  return (
    <div
      className={`fixed top-5 right-5 p-4 rounded-lg shadow-lg flex items-center ${bgColor} ${textColor} z-50`}
    >
        ...
        <div className="p-6 border-b bg-gray-50">
          <h2 className="text-2xl font-bold text-gray-800 flex items-center">
            {type === "PD" ? (
              ...
              <UserSquare className="w-6 h-6 mr-2" />
            )}
        ...
        <tbody className="divide-y divide-gray-200">
          {data.map((item: any) => (
            <tr key={item.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-6 py-4 text-sm text-black max-w-[200px] whitespace-nowrap overflow-hidden text-ellipsis">
                {item.alamat}
              </td>
              {type === "PD" && (
                <td className="px-6 py-4 text-sm max-w-xs truncate text-black">
                  {item.program}
                </td>
              )}
              <td className="px-6 py-4 text-sm text-black">
                {type === "PD" ? item.namaWali : item.email}
              </td>
              <td className="px-6 py-4">
                <select
                  value={item.status}
                  onChange={(e) =>
                    onStatusChange(item.id, e.target.value as DbStatus)
                  }
                  className={`font-medium text-black w-full p-2 text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500
                    ${
                      item.status === "MENUNGGU VERIFIKASI"
                        ...
                        : item.status === "TELAH DIVERIFIKASI"
...
const DetailPendaftaranPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const endpoint =
        tab === "PD"
          ? "/api/pendaftar-peserta-didik"
          : "/api/pendaftar-tenaga-pendidik";
      try {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setNotification({
        message: `Data "${deleteTarget.name}" berhasil dihapus.`,
        type: "success",
      });
    } catch (err: any) {
      setNotification({ message: err.message, type: "error" });
  ...
  const pendingCount = peserta.filter(
    (p) => p.status === "MENUNGGU VERIFIKASI"
  ).length;
  const verifiedCount = peserta.filter(
    (p) => p.status === "TELAH DIVERIFIKASI"
  ).length;
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="bg-white rounded-lg border p-6">
          ...
          <p className="text-gray-600 mt-2">
            Daftar peserta didik dan tenaga pendidik yang telah mendaftar.
          </p>
        </div>
        ...
        {!detailId && (
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border">
              ...
              <div className="text-sm text-gray-600">Telah Diverifikasi</div>
            </div>
            <div className="bg-white rounded-xl p-4 shadow-sm border">
